...
class MORSpyMaster(nn.Module):
    ...
    def _get_primary_reward(self, pos_scores: Tensor, neg_scores: Tensor, neut_scores: Tensor, assas_scores: Tensor, reverse=False):
        ...
        if reverse:
            ...
            return pos_reward.shape[1] - num_correct
    ...
    def wasserstein_rot(self, pos_dist, pos_weights, neg_dist, neg_weights):
        ...
        cdf1 = torch.zeros(len(unique_values), dtype=torch.float)
        cdf2 = torch.zeros(len(unique_values), dtype=torch.float)

        cdf1[indices[:len(pos_dist)]] = pos_weights
        cdf2[indices[len(pos_dist):]] = neg_weights

        # Compute the Wasserstein distance
        distance = torch.sum(torch.abs(cdf1 - cdf2) * torch.diff(unique_values))

        return distance
    
    def _find_search_embeddings(self, reward, word_embeddings):
