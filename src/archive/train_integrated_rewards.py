...
from models.multi_objective_models import MORSpyIntegratedRewards
...
def init_hyperparameters(hp: HyperParameters, model: MORSpyIntegratedRewards, device, normalize_reward):
...
@torch.no_grad()
def validate(model: MORSpyIntegratedRewards, valid_loader: DataLoader, loss_fn: RewardSearchLoss, device: torch.device):
    ...
    model.update_weights(0.0, 1.0, -10.0)
...
def train(hprams: HyperParameters, model: MORSpyIntegratedRewards, train_loader: DataLoader, valid_loader: DataLoader, device: torch.device, normalize_reward: bool) -> TrainLogger:
    ...
    for epoch in range(1, hprams.n_epochs + 1):
        ...
        neg_num = -1
        neut_num = -1
        neut_weight = [random.uniform(-10, 10) for _ in range(len(train_loader))]
        neg_weight = [random.uniform(-10, 10) for _ in range(len(train_loader))]
        assas_weight = [random.uniform(-10, 10) for _ in range(len(train_loader))]
        for i, data in enumerate(train_loader, 0):
            model.update_weights(neg_weight[i], neut_weight[i], assas_weight[i])
            if (i % 100 == 0):
...
def main(args):
    ...
    model = MORSpyIntegratedRewards(vector_db, device, neutral_weight=hpram.neut_weight, negative_weight=hpram.neg_weight, assas_weights=hpram.assas_weight, vocab_size=hpram.search_window)
